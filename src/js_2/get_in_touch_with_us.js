...
iName.addEventListener('input', function() {
  ...
  if (inputValue.length < 3 || !/^[a-zA-Z]+$/.test(inputValue)) {
    iName.setCustomValidity('The name must be at least 3 letters without spaces or other symbols.');
  } else {
...
iPhone.addEventListener('input', function() {
  ...
  if (!/^\d{9}$/.test(inputValue)) {
    iPhone.setCustomValidity('Please enter a valid English mobile number (9 digits).');
  } else {
